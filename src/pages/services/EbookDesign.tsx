...
const EbookDesign = () => {
  ...
  const packages = [
    { name: 'Starter', price: '$299', description: 'Perfect for short ebooks and lead magnets.', features: ['Up to 30 pages', 'Basic layout design', 'Cover design', 'PDF format', 'One revision round', '7-day delivery'], isPopular: false },
    { name: 'Professional', price: '$799', description: 'Ideal for full-length ebooks and guides.', features: ['Up to 100 pages', 'Premium layout design', 'Custom cover design', 'PDF, EPUB & MOBI formats', 'Interactive elements', 'Three revision rounds', 'Custom graphics included', '14-day delivery'], isPopular: true },
    { name: 'Enterprise', price: '$1,499', description: 'Complete solution for book series or complex projects.', features: ['Unlimited pages', 'Bespoke layout design', 'Premium cover + variations', 'All digital formats', 'Print-ready files', 'Unlimited revisions', 'Custom illustrations', 'ISBN & metadata setup', 'Priority 10-day delivery'], isPopular: false },
  ];

  const deliverables = [
    'Professional Layout Design', 'Custom Cover Artwork', 'Table of Contents', 'Chapter Heading Designs',
    'PDF Export (Web & Print)', 'EPUB & MOBI Formats', 'Source Files (upon request)', 'Style Guide Document',
  ];
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container-custom relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="hero-content">
              <span className="inline-block px-4 py-2 rounded-full glass text-amber-500 text-sm font-medium mb-6">Ebook Design</span>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-heading font-bold mb-6">Professional <span className="text-amber-500">Ebook Design</span> That Sells</h1>
              <p className="text-lg text-muted-foreground mb-8">Transform your content into beautifully designed ebooks that captivate readers and elevate your brand. From lead magnets to full-length books.</p>
          ...
          <div className="features-grid grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => (
              <div key={index} className="feature-card glass p-6 rounded-2xl hover:scale-105 transition-transform duration-300">
                <div className="w-12 h-12 rounded-xl bg-amber-500 flex items-center justify-center mb-4"><feature.icon className="w-6 h-6 text-white" /></div>
        ...
        <div className="container-custom">
          ...
          <div className="pricing-grid grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {packages.map((pkg, index) => (
              <div key={index} className={`pricing-card relative p-8 rounded-3xl transition-all duration-300 hover:scale-105 ${pkg.isPopular ? 'bg-card border-2 border-amber-500 shadow-2xl' : 'glass border border-border'}`}>
                {pkg.isPopular && (
                ...
                <ul className="space-y-3 mb-8">
                  {pkg.features.map((feature, i) => (
                    <li key={i} className="flex items-center gap-3">
                      <div className="w-5 h-5 rounded-full bg-amber-500/20 flex items-center justify-center"><Check className="w-3 h-3 text-amber-500" /></div>
                      <span className="text-sm">{feature}</span>
                    </li>
                  ))}
                </ul>
                <Button onClick={() => handleGetQuote(pkg.name)} className={`w-full ${pkg.isPopular ? 'bg-amber-500 hover:bg-amber-600 text-white' : ''}`} variant={pkg.isPopular ? 'default' : 'outline'} size="lg">Get Quote</Button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-muted-foreground mb-8">Every ebook project includes comprehensive deliverables to ensure your content looks professional across all platforms and devices.</p>
              <div className="grid grid-cols-2 gap-4">
                {deliverables.map((item, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-full bg-amber-500 flex items-center justify-center"><Check className="w-3 h-3 text-white" /></div>
            ...
            <div className="glass p-8 rounded-2xl">
              ...
              <p className="text-muted-foreground mb-6">Have a unique project or specific requirements? We'd love to hear about it and provide a tailored solution.</p>
              <Link to="/contact#contact-form"><Button className="w-full bg-amber-500 hover:bg-amber-600 text-white">Contact Us</Button></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container-custom text-center">
          ...
          <p className="text-muted-foreground mb-8 max-w-xl mx-auto">Let's transform your content into a beautifully designed ebook that readers will love.</p>
          <Button size="lg" className="bg-amber-500 hover:bg-amber-600 text-white" onClick={() => handleGetQuote('Professional')}>Start Your Ebook Project</Button>
        </div>
      </section>

      <Footer />
      <QuoteModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} serviceName="Ebook Design" packageName={selectedPackage} />
    </div>
  );
};
